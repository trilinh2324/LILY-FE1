...
const Buy = () => {
    ...
    const calculateTotal = (items, method) => {
        ...
        // Tính tổng tiền phải trả dựa trên phương thức vận chuyển
    ...
    const handleOrderSubmit = async () => {
        ...
        try {
            ...
            toast.success('Đơn hàng đã được gửi thành công!', { autoClose: 2000 });
            await Promise.all(cartItems.map(item => 
                axios.delete(`http://localhost:8090/api/cart/removeFromCart/${item.id}`)
            ));
             navigate('/bill');
        } catch (error) {
            toast.error('Có lỗi xảy ra khi gửi đơn hàng.', { autoClose: 2000 });
